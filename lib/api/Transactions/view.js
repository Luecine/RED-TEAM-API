...
router.post("/search", validateUserToken, async (req, res) => {
    ...
    const results = await Model.sequelize.query(
        `SELECT DISTINCT * FROM transactions WHERE sendtime >= '${startDate}' AND sendtime<= '${endDate}'`
    );

    const [returndata] = results;
    var list = [];
    returndata.forEach(function (i) { 
    if((i.from_account == account_number ) || ( i.to_account == account_number.toString())){
